defmodule Aoc.Year2022.Day05.SupplyStacks do
  ...
  *After the rearrangement procedure completes, what crate ends up on top of each
  ...
  def part_2(input) do
    input
    |> parse()
    |> solve(&multiple_move/2)
  end

  defp solve({columns, algorithm}, move_fn) do
    Enum.reduce(algorithm, columns, move_fn)
    |> Enum.map(&List.first/1)
    |> Enum.join()
  end

  defp single_move({how_many, from, to}, columns) do
    Enum.reduce(
      1..how_many,
      columns,
      fn _, state ->
        [moved_crate | from_column] = Enum.at(state, from - 1)
        to_column = [moved_crate | Enum.at(state, to - 1)]

        state
        |> List.update_at(from - 1, fn _ -> from_column end)
        |> List.update_at(to - 1, fn _ -> to_column end)
      end
    )
  end

  defp multiple_move({how_many, from, to}, columns) do
    {taken, left} = columns |> Enum.at(from - 1) |> Enum.split(how_many)

    columns
    |> List.update_at(from - 1, fn _ -> left end)
    |> List.update_at(to - 1, &(taken ++ &1))
  end

  # Parsing functions

  defp parse(input) do
    [initial_state, algorithm] =
      input
  ...
  defp parse_columns(state) do
    ...
    |> Enum.map(&Enum.filter(&1, fn spot -> spot != " " end))
  end
end
