defmodule Aoc.Year2022.Day07.NoSpaceLeftOnDevice do
  ...
  *total size* of each directory. The total size of a directory is the sum of the
  ...
  def part_2(input) do
    ...
    %{directories: directories} = parse(input)

    current_free_space = total_disk_space - Map.get(directories, [])
  ...
  def parse(input) do
    ...
    |> String.split("\n", trim: true)
    |> Enum.drop(1)
    |> Enum.reduce(%{path: [], directories: %{}}, &parse_commands/2)
  end

  def parse_commands("$ cd ..", %{path: []} = fs), do: fs

  def parse_commands("$ cd ..", %{path: [_ | path]} = fs), do: %{fs | path: path}

  def parse_commands("$ cd " <> directory, %{path: path} = fs) do
    %{fs | path: [directory | path]}
  end

  def parse_commands("dir " <> directory, %{directories: directories, path: path} = fs) do
    %{fs | directories: Map.put_new(directories, [directory | path], 0)}
  end

  def parse_commands("$" <> _, fs), do: fs

  def parse_commands(file_size, path) do
    calculate_size(path, Integer.parse(file_size) |> elem(0))
  end

  def calculate_size(%{path: path} = fs, size) when is_integer(size) do
    do_calculate_size(fs, path, size)
  end

  def do_calculate_size(%{directories: directories} = fs, [_ | rest] = path, size) do
    fs
    |> Map.put(:directories, Map.update(directories, path, size, &(&1 + size)))
    |> do_calculate_size(rest, size)
  end

  def do_calculate_size(%{directories: directories} = fs, [], size) do
    %{fs | directories: Map.update(directories, [], size, &(&1 + size))}
  end
end
